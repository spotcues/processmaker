...
use WsBase;
use AppCacheView;
use ProcessMaker\Util\LiteUtil;
/**
 *
...
class Lite extends Api
{
    ...
    function addCaseInfo($response, $caseDetails){
        $response["caseCurrentState"] = $caseDetails["APP_STATUS"];
        $response["caseId"] = $caseDetails["APP_UID"];
        $response["caseInitDetails"] = array();
        $response["caseInitDetails"]["caseInitDate"] = $caseDetails["APP_INIT_DATE"];
        $response["caseInitDetails"]["caseInitUser"] = $caseDetails["APP_INIT_USER"];
        $response["caseNumber"] = $caseDetails["APP_NUMBER"];
        $response["caseStatus"] = $caseDetails["APP_STATUS"];
        $response["variables"] = $caseDetails["APP_DATA"];
        return $response;
    }

    /**
     * Get case by Id and Status
     * @return array
     *
     * @copyright Spotcues
     *
     * @url GET /get-case-by-status/:app_uid/:status
     * @param string $app_uid {@min 32}{@max 32}
     * @param string $status
     */
    public function getCaseByIdAndStatus($app_uid, $status){
        $app_cache_view = new AppCacheView();
        $response = $app_cache_view->getCurrentUsersAndCaseInfo($app_uid);
        $userUid = $this->getUserId();
        $case = new ClassesCases();
        $caseDetails = $case->loadCase($app_uid);
        $response["_caseStatus"] = $status;
        $response = $this->addCaseInfo($response, $caseDetails);
        # Inject Case History
        $caseHistory = $this->doGetCasesListHistory($app_uid);
        $response['_history'] = $caseHistory->data;
        return $response;
    }
    ...
    public function doGetCasesListToDo(
        ...
        try {
            ...
            // Add history info
            $result = $this->addAdditionalCaseInfo($result, "TO_DO");
    ...
    public function addAdditionalStepInfo($data, $pro_uid, $act_uid, $app_uid, $app_index, $steps){
        /**
             * {
             *      totalSteps: <number>
             *      currentStep: <number>
             ...
             *      caseId: <app-uid>
             *      caseIndex: <del-index>
             *      formData: <dyanform-content>
             *      variables: <case-variables>
             * }
        */
        $data['_stepInfo']['totalSteps'] = count($steps);
        $dyn_uid = $data["conditionalSteps"]["UID"];
        $step_pos = $data["conditionalSteps"]["POSITION"];
        $data['_stepInfo']['currentStep'] = $step_pos;
        $data['_stepInfo']['stepId'] = $steps[$step_pos - 1]["stepId"];
        $data['_stepInfo']['caseId'] = $app_uid;
        // Add case_status
        $case = new ClassesCases();
        $caseDetails = $case->loadCase($app_uid, $app_index);
        $data['_stepInfo']['caseInitUser'] = $caseDetails['APP_INIT_USER'];
        $data['_stepInfo']['caseStatus'] = $caseDetails['APP_STATUS'];
        $data['_stepInfo']['caseIndex'] = $app_index;
        $data['_stepInfo']['formData'] = $this->doGetDynaForm($pro_uid, $dyn_uid);
        $caseVars = $caseDetails["APP_DATA"];
        $caseVars = DateTime::convertUtcToTimeZone($caseVars);
        $data['_stepInfo']['variables'] = $caseVars;
        return $data;
    }

    public function addAdditionalCaseInfo($data, $caseListType)
    {
        $ws = new WsBase();
        $curDueDate = "";
        foreach ($data as $key => $value) {
            $data[$key]['currentUsers'] = [];
            $fields = $ws->getCaseInfo($value['caseId'], $value['delIndex']);
            foreach ($fields->currentUsers as $key2 => $value2) {
                $data[$key]['currentUsers'][] = $value2->userId;
                $curDueDate = $value2->delTaskDueDate;
            }

            # override the dueDate for participated listing to indicate the current task dueDate
            if($caseListType === "PARTICIPATED"){
                if(!empty($curDueDate)){
                    $data[$key]['dueDate'] = $curDueDate;
                }
            }

            # Inject Case History
            $caseHistory = $this->doGetCasesListHistory($value['caseId']);
            $data[$key]['_history'] = $caseHistory->data;

            # Inject formdata of first step if caseStatus is "DRAFT" or "TO_DO"
            #  - Execute doGetStep for the initial step
            #  - Get dynaform Id
            #  - Get dynaform data
            if($caseListType === "DRAFT" || $caseListType === "TO_DO"){
                $pro_id = $value['process']['processId'];
                $task_id = $value['task']['taskId'];
                $request_data["pro_uid"] = $pro_id;
                $request_data["act_uid"] = $task_id;
                $request_data["step_uid"] = null;
                $request_data["step_pos"] = null;
                $request_data["app_index"] = $value['delIndex'];
                $getStepResp = $this->doGetStep($value['caseId'], $request_data);
                $data[$key]['_getStepResp'] = $getStepResp;
                $step_type = $getStepResp['conditionalSteps']['TYPE'];
                if($step_type === "DYNAFORM"){
                    $dyn_id = $getStepResp['conditionalSteps']['UID'];
                    $data[$key]['_formData'] = $this->doGetDynaForm($pro_id, $dyn_id);
                }
                $data[$key]['_steps'] = $this->doGetActivitySteps($task_id, $pro_id);
            }
            # Inject case variables
            $caseVariables = $this->doGetCaseVariables($value['caseId']);
            $data[$key]['_caseVariables'] = $caseVariables;
        }

        return $data;
    }

    public function addAdditionalCaseInfoV2($data, $caseListType)
    {
        $ws = new WsBase();
        $curDueDate = "";
        foreach ($data as $key => $value) {
            $data[$key]['currentUsers'] = [];
            $fields = $ws->getCaseInfo($value['caseId'], $value['delIndex']);
            $data[$key]['variables'] = $fields->appData;
            $data[$key]['caseInitUser'] = $fields->caseCreatorUser;
            $data[$key]['caseInitDate'] = $fields->createDate;
            foreach ($fields->currentUsers as $key2 => $value2) {
                $data[$key]['currentUsers'][] = $value2->userId;
                $curDueDate = $value2->delTaskDueDate;
            }
            ...
            if($caseListType === "DRAFT" || $caseListType === "TO_DO"){
                ...
                $task_id = $value['task']['taskId'];
                $reqData = array();
                $reqData["pro_uid"] = $pro_id;
                $reqData["app_index"] = $value['delIndex'];
                $usr_uid = $this->getUserId();
                $liteUtil = new LiteUtil();
                $getStepResp = $liteUtil->doGetStepStandalone($value['caseId'], $reqData, $usr_uid);
                $data[$key]['_stepInfo'] = $getStepResp['_stepInfo'];
            }
        }
    ...
    public function addTaskProperties($data)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            list($data[$key]['TAS_OFFLINE'], $data[$key]['TAS_AUTO_ROOT'],$data[$key]['TAS_DESCRIPTION']) = $task->getColumnValues($valueUpper['PRO_UID'],
                $valueUpper['TAS_UID'], array('TAS_OFFLINE', 'TAS_AUTO_ROOT','TAS_DESCRIPTION'));
        }
    ...
    public function parserDataTodo($data)
    {
        $structure = array(
            ...
            'APP_INIT_USER' => 'caseInitUser',
            'APP_INIT_DATE' => 'caseInitDate',
            'user' => array(
                'USR_UID' => 'userId'
            ),
            'prevUser' => array(
    ...
    /**
     * Get list Case MyTask
     * 
     * @copyright Spotcues
     * 
     * @url GET /mytask/v2
     */
    public function doGetCasesV2MyTasks($start = 0,
    $limit = 10,
    $sort = 'DEL_DELEGATE_DATE',
    $dir = 'DESC',
    $cat_uid = '',
    ...
    $isDraftEnabled = false
        ...
        try {
            $isDraftEnabled = isset($isDraftEnabled) && !empty($isDraftEnabled)? $isDraftEnabled: false;
            if (preg_match($this->regexNull, $newerThan)) {
                return [];
            }
            ...
            $usr_uid = $this->getUserId();
            $app_cache_view = new AppCacheView();
            $result = $app_cache_view->getCasesOfUserWithCaseInfoByAction("mytask",$usr_uid, $start, $limit, $oldestthan, $isDraftEnabled);
            $result["cases"] = $this->addAdditionalCaseInfoV2($result["cases"], 'TO_DO');
            return $result;
        } catch (Exception $e) {
            throw (new RestException(Api::STAT_APP_EXCEPTION, $e->getMessage()));
        }
     ...
     * @url GET /todo/v2
     */
    public function doGetCasesV2Todo($start = 0,
    $limit = 10,
    $sort = 'DEL_DELEGATE_DATE',
    $dir = 'DESC',
    $cat_uid = '',
    ...
    $oldestthan = ''){
        try {
            ...
            $usr_uid = $this->getUserId();
            $app_cache_view = new AppCacheView();
            $result = $app_cache_view->getCasesOfUserWithCaseInfoByAction("todo",$usr_uid, $start, $limit, $oldestthan);
            $result["cases"] = $this->addAdditionalCaseInfoV2($result["cases"], 'TO_DO');
            return $result;
        } catch (Exception $e) {
            throw (new RestException(Api::STAT_APP_EXCEPTION, $e->getMessage()));
    ...
    /**
     * Get list Case Unassigned
     * 
     * @copyright Spotcues
     * 
     * @url GET /unassigned/v2
     */
    public function doGetCasesV2UnAssigned($start = 0,
    $limit = 10,
    $sort = 'DEL_DELEGATE_DATE',
    $dir = 'DESC',
    $cat_uid = '',
    ...
    $oldestthan = ''){
        try {
            ...
            $usr_uid = $this->getUserId();
            $app_cache_view = new AppCacheView();
            $result = $app_cache_view->getCasesOfUserWithCaseInfoByAction("unassigned", $usr_uid, $start, $limit, $oldestthan);
            $result["cases"] = $this->addAdditionalCaseInfoV2($result["cases"], 'UN_ASSIGNED');
            return $result;
        } catch (Exception $e) {
            throw (new RestException(Api::STAT_APP_EXCEPTION, $e->getMessage()));
        }
    ...
    /**
     * Get list Case Participated
     * 
     * @copyright Spotcues
     * 
     * @url GET /participated/v2
     */
    public function doGetCasesV2Participated($start = 0,
    $limit = 10,
    $sort = 'DEL_DELEGATE_DATE',
    $dir = 'DESC',
    $cat_uid = '',
    ...
    $status=''
        ...
        try {
            ...
            $usr_uid = $this->getUserId();
            $app_cache_view = new AppCacheView();
            $result = $app_cache_view->getCasesOfUserWithCaseInfoByAction("sent", $usr_uid, $start, $limit, $oldestthan, $isDraftEnabled, $status);
            $result["cases"] = $this->addAdditionalCaseInfoV2($result["cases"], 'PARTICIPATED');
            return $result;
        } catch (Exception $e) {
            throw (new RestException(Api::STAT_APP_EXCEPTION, $e->getMessage()));
        }
     ...
     * @url POST /cced/v2
     */
    public function doGetCasesV2CCed($request_data){
        try {
            $appUids = $request_data['appUids'];
            $app_cache_view = new AppCacheView();
            $result = $app_cache_view->getCasesWithCaseInfo($appUids);
            $result["cases"] = $this->addAdditionalCaseInfoV2($result["cases"], 'CCed');
            return $result;
        } catch (Exception $e) {
            throw (new RestException(Api::STAT_APP_EXCEPTION, $e->getMessage()));
        }
     ...
     * @url GET /draft/v2
     */
    public function doGetCasesV2Draft($start = 0,
    $limit = 10,
    $sort = 'DEL_DELEGATE_DATE',
    $dir = 'DESC',
    $cat_uid = '',
    ...
    $oldestthan = ''){
        try {
            ...
            $usr_uid = $this->getUserId();
            $app_cache_view = new AppCacheView();
            $result = $app_cache_view->getCasesOfUserWithCaseInfoByAction("draft",$usr_uid, $start, $limit, $oldestthan);
            $result["cases"] = $this->addAdditionalCaseInfoV2($result["cases"], 'TO_DO');
            return $result;
        } catch (Exception $e) {
            throw (new RestException(Api::STAT_APP_EXCEPTION, $e->getMessage()));
    ...
    public function doGetCasesListDraft(
        ...
        try {
            ...
            // Add history info
            $result = $this->addAdditionalCaseInfo($result, "DRAFT");
    ...
    public function doGetCasesListParticipated(
        ...
        try {
            ...
            // Add history info
            $result = $this->addAdditionalCaseInfo($result, "PARTICIPATED");
    ...
    public function doGetCasesListUnassigned(
        ...
        try {
            ...
            $response = $this->addTaskProperties($response);
            $result = $this->parserDataUnassigned($response);

            // Add history info
            $result = $this->addAdditionalCaseInfo($result, "UNASSIGNED");
    ...
    public function addAdditionalStartCaseInfo($result, $pro_uid, $task_uid){
        $reqData['pro_uid'] = $pro_uid;
        $reqData['act_uid'] = $task_uid;
        $reqData['app_index'] = $result['caseIndex'];
        $reqData['step_uid'] = null;
        $reqData['step_pos'] = 0;
        $reqData['app_index'] = $result['caseIndex'];
        $app_uid = $result['caseId'];
        $result['initialStep'] = $this->doGetStep($app_uid, $reqData);
        return $result;
    }
     ...
     * @class AccessControl {@permission PM_CASES}
     */
    public function postStartCase($pro_uid, $task_uid, $request_data)
    {
        try {
            $oMobile = new BusinessModelLight();
            $customAttributes = $request_data['customAttributes'];
            $result = $oMobile->startCase($this->getUserId(), $pro_uid, $task_uid);
            if(isset($customAttributes) && !empty($customAttributes)){
                // create the customAttributes as caseVariables.
                $this->doPutCaseVariables($result["caseId"], $customAttributes);
            }
            $result = $this->addAdditionalStartCaseInfo($result, $pro_uid, $task_uid);
        } catch (Exception $e) {
            throw (new RestException(Api::STAT_APP_EXCEPTION, $e->getMessage()));
        }
    ...
    public function doGetStep($app_uid, $request_data)
    {
        ...
        $step_uid = isset($request_data["step_uid"]) && !empty($request_data["step_uid"])? $request_data["step_uid"] : null;
        $step_pos = isset($request_data["step_pos"]) && !empty($request_data["step_pos"])? $request_data["step_pos"] : 0;
        $app_index = isset($request_data["app_index"]) && !empty($request_data["app_index"])? $request_data["app_index"] : null;
        $dyn_uid = isset($request_data["dyn_uid"]) && !empty($request_data["dyn_uid"])? $request_data["dyn_uid"] : null;
        $usr_uid = $this->getUserId();
        $steps = $this->doGetActivitySteps($act_uid, $pro_uid);
        if($step_pos > count($steps)){
            throw (new RestException(Api::STAT_APP_EXCEPTION, "The step position is incorrect"));
        }
        if(!isset($app_index) || empty($app_index) || is_null($app_index)){
            // Add case_index for the current user
            $app_cache_view = new AppCacheView();
            $app_index = $app_cache_view->getCurrentDelIndexOfUserForCase($usr_uid, $app_uid);
        }
        ...
        $oMobile = new BusinessModelLight();
        if($step_pos - 1 < count($steps) && $step_pos - 1 >= 0){
            // execute triggers after for previous step
            $triggers = $oMobile->doExecuteTriggerCase($usr_uid, $pro_uid, $act_uid, $app_uid, $steps[$step_pos - 1]["stepId"],
                    "after", $app_index);
        }
        //trigger before
        if ($conditionalSteps) {
        ...
        $step_pos = $response["conditionalSteps"]["POSITION"];
        if($step_pos - 1 >= 0 && $step_pos - 1 < count($steps)){
            $response = $this->addAdditionalStepInfo($response, $pro_uid, $act_uid, $app_uid, $app_index, $steps);

            return $response;
        }else{
            // finished all the steps in the current task. Ready for routing
            throw (new RestException(204, "The case shall be routed"));
        }
    }
