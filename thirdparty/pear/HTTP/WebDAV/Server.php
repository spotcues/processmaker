...
class HTTP_WebDAV_Server
{
    ...
    function ServeRequest()
    {
        ...
        $messagePath = (@$_SERVER["HTTPS"] === "on" ? "https:" : "http:");
        ...
        if ( substr ( $requestUri, -1 ) == '/' ) $requestUri = substr ( $requestUri, 0, strlen ($requestUri) -1 );
        ...
        array_shift ( $paths );
        array_shift ( $paths );
        array_shift ( $redirectPaths );
        array_pop   ( $redirectPaths );
        $this->paths = $paths;
        ...
        if (!strlen($this->path)) {
            if ($_SERVER["REQUEST_METHOD"] == "GET") {
                ...
                $redirectPaths = implode ('/', $redirectPaths );
                $messagePath  .= '//'.$_SERVER['HTTP_HOST'].'/'.$redirectPaths.'/unavailableService';
                header("Location: ".$messagePath);
    ...
    */
    ...
    function http_PROPFIND()
    {
        ...
        foreach($files["files"] as $file) {
            ...
            $href = $this->_mergePathes($this->uriBase, $path);
