...
{opt={fx:true,x:this.options.position.x,y:this.options.position.y}.concatMaborak(opt);this.options.position.x=opt.x;this.options.position.y=opt.y;if(opt.fx===true)
...
{var hj=(parseInt(this.parent.dom.getStyle(tb,"width"),10)-((!this.parent.browser.isIE)?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?0:3):0));this.parent.dom.setStyle(tb,{width:hj,borderLeftWidth:4});}
else
{this.parent.dom.setStyle(tb,{});}}
tb.onmouseup=function(){return false;};if(this.tabLastSelected!==false)
{var tls=this.elements.tabOptions[this.tabLastSelected];tls.className="panel_tabOption"+thm+"___"+this.getTheme("tabOption");tls.onmouseover=function(o,j){o=window.event?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?j:o):j;o.a.className=o.b;}.args({a:tls,b:"panel_tabOptionOver"+thm+"___"+this.getTheme("tabOptionOver")});tls.onmouseout=function(o,j){o=window.event?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?j:o):j;o.a.className=o.b;}.args({a:tls,b:"panel_tabOption"+thm+"___"+this.getTheme("tabOption")});tls.onmouseup=function(event,tabID){if(this.tab.manualDisabled){return false;}
this.tabSelected=(this.parent.browser.isIE)?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?tabID:event):tabID;this.makeTab();return false;}.extend(this,this.tabLastSelected);if(this.tab.display==="vertical")
{this.parent.dom.setStyle(tls,{width:parseInt(this.parent.dom.getStyle(tb,"width"),10)+((!this.parent.browser.isIE)?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?0:3):0),borderLeftWidth:1});}
else
...
{options={proxy:false}.concatMaborak(options||{});this.clearContent();if(options.proxy===false)
{if(options.url)
...
{this.parent.dom.setStyle(this.elements.frontend,{width:(this.parent.browser.isIE)?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?"100%":"auto"):"100%"});this.parent.dom.setStyle(this.elements.frontend,this.setStyle.frontend||{});},backend:function()
...
{this.options.concatMaborak(this.coords());this.direction=this.getDirection();this.lines=this.createLines();this.elements=[];for(var i=0;i<5;i++)
...
{if(this.changed())
{this.options.concatMaborak(this.coords());if(this.kase()%2===0)
...
{this.options={bubble:true,theme:"firefox"}.concatMaborak(options||{});if(!this.validate()){return false;}
...
dv.onmouseover=function(evt,el){dv=(this.parent.browser.isIE?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?el:evt):el);var i=dv.i;dv.a.className="app_menuRight_option_over___"+this.options.theme;dv.b.className="app_menuRight_option_image_over___"+this.options.theme;if(this.submenu&&this.submenu_current_i!=i)
...
this.remove();opt=this.parent.browser.isIE?((!!navigator.userAgent.match(/Trident.*rv[ :]*11\./))?opt:evt):opt;var lch=this.options.menu[opt];if(lch&&typeof lch.launch=="function")
...
{this.options={inProgress:function(){},callback:function(){}}.concatMaborak(options||{});if(!this.parent.dom.element(this.options.form)){return false;}
...
{this.options={initIn:0,counter:true,target:document.body,resize:true,size:{w:400,h:310},position:{x:0,y:0,center:true},images:[]}.concatMaborak(options||{});this.windowImage();this.image=$dce("img");this.image.id=this.id;this.panel.elements.content.appendChild(this.image);this.load(this.options.initIn);};this.windowImage=function()
...
{this.interval=false;this.options={url:false,method:"GET",args:""}.concatMaborak(options||{});this.begin=new Date().getTime();this.tmp="rpcJson_"+this.begin;this.server=this.parent.info.base+"server/proxy.js.php";this.par=this.parent.info.domBaseJS.parentNode;this.make=function(options)
...
{this.make=function(options)
{this.options={transition:"sineInOut",duration:2000,fps:70,onTransition:function(){},onFinish:function(){},begin:0,end:100,timeBegin:new Date().getTime()}.concatMaborak(options||{});this.timer=setInterval(function(){var time=new Date().getTime();if(time<this.options.timeBegin+this.options.duration)
...
{this.make=function(options)
{this.options={duration:1000,transition:"sineInOut",end:0,onFinish:function(){}}.concatMaborak(options);this.dom=((this.options.dom||[]).isArray)?this.options.dom:[this.options.dom];for(var i=0;i<this.dom.length;i++)
...
{this.make=function(options)
{this.options={duration:1000,transition:"sineInOut",end:0,onFinish:function(){}}.concatMaborak(options);this.dom=this.options.dom;new this.parent.module.fx.algorithm().make({duration:this.options.duration,end:[this.options.end.x,this.options.end.y],transition:this.options.transition,begin:[parseInt(this.dom.style.left),parseInt(this.dom.style.top)],onTransition:function(fx,dom){this.dom.style.left=fx.result[0];this.dom.style.top=fx.result[1];}.extend(this,this.dom),onFinish:function(fx,dom,finish)
...
leimnud.Package.Public({info:{Class:"maborak",File:"module.drag.js",Name:"drag",Type:"module",Version:"0.1"},content:function(options){this.options={limitbox:false}.concatMaborak(options||{});this.loaded=false;this.eventHandlers=[];this.cursor="move";this.uid=this.parent.tools.createUID();this.make=function()
{this.options.elements=this.set();this.options.fx={type:"simple",target:document.body}.concatMaborak(this.options.fx||{});this.events=this.events||{};var elements=(this.options.elements||[]).length;this.flagEvents=[];var oThis=this;for(var i=0;i<elements;i++)
...
{this.button=(label&&label.tagName)?$(label):(new this.parent.module.dom.create("input",{className:"module_app_button___gray",type:"button",value:label||"Button"}.concatMaborak(prop||{}),style||{}));this.button.disable=function()
...
{this.make=function(options)
{this.input=(options&&options.tagName)?$(options):(new this.parent.module.dom.create("input",{className:"module_app_input___gray",type:"text",value:options.label||"",maxLength:options.maxlength||"30"}.concatMaborak(options.properties||{}),(options.style||{})));this.input.disable=function()
...
{this.options={data:[],selected:0,properties:{},style:{}}.concatMaborak(options||{});this.make=function()
...
{data={value:null,text:null,selected:false,key:false}.concatMaborak(data||{});var o=new Option(data.text,data.value,data.selected);if(data.key===false)
...
{this.options={drag:true,panel:[],data:[]}.concatMaborak(options||{});this.drop=new this.parent.module.drop();this.drop.make();var width=this.options.target.offsetWidth-50;this.columns=this.options.data.length;this.widthColumn=(width/this.columns);this.elements.column=[];this.elements.table=$dce('table');$(this.elements.table).setStyle({width:width,borderCollapse:'collapse'})
...
{var wd=column[j];this.panel({target:this.elements.column[i],column:i,index:j}.concatMaborak(wd));this.matriz[i].push(j);}}};this.panel=function(options)
{options={style:{},titleBar:true}.concatMaborak(options||{});var _panel=new this.parent.module.panel();_panel.options={target:options.target,title:options.title||"",size:{w:(options.width||this.widthColumn),h:options.height||300},position:{x:0,y:0},titleBar:(options.titleBar||false),control:{resize:false,roll:false,setup:false,drag:this.options.drag,close:true},fx:{shadow:false,opacity:false}};_panel.setStyle={containerWindow:(options.style.containerWindow||{}).concatMaborak({position:'relative',border:"1px solid #afafaf",margin:3}),content:(options.style.content||{}).concatMaborak({overflow:"hidden",margin:0,borderLeftWidth:0,borderRightWidth:0,borderBottomWidth:0}),titleBar:(options.style.titleBar||{}).concatMaborak({backgroundImage:"url("+this.parent.info.images+"grid.title.gray.gif)",height:16,backgroundPosition:"0pt -5px"}),title:(options.style.title||{}).concatMaborak({padding:1,fontWeight:"normal"}),roll:(options.style.roll||{}).concatMaborak({top:1}),close:(options.style.close||{}).concatMaborak({top:1}),setup:(options.style.setup||{}).concatMaborak({top:1})};if(options.noBg)
{_panel.setStyle.content.concatMaborak({backgroundColor:"#DFDFDF",borderWidth:0});_panel.setStyle.containerWindow.concatMaborak({backgroundColor:"#DFDFDF"});_panel.setStyle.frontend={backgroundColor:"#DFDFDF"};}
...
{try{if(typeof(parent)!="undefined"&&typeof(parent.parent)!="undefined"&&typeof(parent.parent.Ext)!="undefined"){if(!parent.parent.PANEL_EAST_OPEN){var debugPanel=parent.parent.Ext.getCmp("debugPanel");parent.parent.PANEL_EAST_OPEN=true;debugPanel.show();debugPanel.ownerCt.doLayout();debugPanel.expand();}
parent.parent.propStore.load();parent.parent.triggerStore.load();}}catch(e){}}
...
{this.panels.editor.loader.hide();this.data.db=xml.xmlhttp.responseText.parseJSON().concatMaborak({});this.data.db.subprocess=[];this.panels.editor.addContentStatus(G_STRINGS.ID_PROCESSMAP_LOADING);if(this.options.rw===true)
...
{options_task={tp:'task'}.concatMaborak(options_task||{});var options=this.data.db['task'][index];if(this.options.rw){options.color=((options_task.tp==='task')?"auto":"#9B88CA");if(options_task.tp==='hidden'){options.color="transparent";}}
...
{this.options={theme:"firefox",rw:true,mi:true,ct:false,hideMenu:true}.concatMaborak(this.options||{});this.options.target=this.parent.dom.element(this.options.target);if(!this.validate()){return false;}
...
{var m=this.menu.cursor;var cpos=this.parent.dom.position(this.panels.editor.elements.content);var index=this.data.db.task.length;var scl={x:this.panels.editor.elements.content.scrollLeft,y:this.panels.editor.elements.content.scrollTop};var pos={x:scl.x+(m.x-cpos.x),y:scl.y+(m.y-cpos.y)};var index=this.data.db.text.length;this.data.db.text[index]={label:text,position:{x:pos.x,y:pos.y},uid:false};this.data.build.text(index);text=text.replace(/"/g,'\\"');var r=new leimnud.module.rpc.xmlhttp({url:this.options.dataServer,args:"action=addText&data="+{uid:this.options.uid,label:encodeURIComponent(text),position:{x:pos.x,y:pos.y}}.toJSONString()});r.callback=function(rpc,index){var rs=rpc.xmlhttp.responseText.parseJSON();this.data.db.text[index].uid=rs.uid;}.extend(this,index);r.make();}}.extend(this)});},addGuide:function(evt,dir)
...
function getPMFolderTags(rootfolder){document.getElementById('tags_cloud').innerHTML="<img src='/images/classic/loader_B.gif' >";var oRPC=new leimnud.module.rpc.xmlhttp({url:'appFolderAjax',async:false,method:'POST',args:'action=getPMFolderTags&rootFolder='+rootfolder});oRPC.make();document.getElementById('tags_cloud').innerHTML=oRPC.xmlhttp.responseText;var scs=oRPC.xmlhttp.responseText.extractScript();scs.evalScript();}
